...
public:
    ...
    myUdpServer(boost::asio::io_service& io_service, short port): socket_(io_service,udp::endpoint(udp::v4(),port))
    {
        my_udp_receive();
    }
    ...
    void my_udp_receive()
    {
        ...
        sender_endpoint_,[this] (boost::system::error_code ec, std::size_t recvd_bytes)
        {
            if(!ec && recvd_bytes > 0 )
            {
    ...
    void my_udp_send_back()
    {
        ...
        socket_.async_send_to(boost::asio::buffer(myStr.c_str(),myStr.length()),
        sender_endpoint_,[this](boost::system::error_code ec , std::size_t recvd_bytes)
        {
            my_udp_receive();
        });
