...
//#include <sys/vtimes.h>   // No search directory
#include <unistd.h>
#include <ulimit.h>
#include <vector>
#include <fstream>
...
{
    char myArray[8192];
    // memset(myArray,'\0',sizeof(myArray)+1); // only to warning
...
int main(int argc , char* argv[])
{
    ...
    if( argc != 3)
    {
        std::cout << " Usage : $ <file-name> <output file> " << std::endl;
        return 1;
    }
    ...
    if( ret == 0)
    {
        ...
        std::cout << pUsg->ru_msgrcv       << " Number of IPC message received  "      << std::endl;
        std::cout << pUsg->ru_msgsnd       << " Number of IPC message sent :  "        << std::endl;
        ...
        std::cout << pUsg->ru_nsignals     << " Number of signal received "            << std::endl;
    ...
    {
        std::cout << " Failed call getrusgae() " << std::endl;
    }
