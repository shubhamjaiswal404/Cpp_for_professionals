...
DOMNodeFilter::FilterAction DOMPrintFilter::DOMPrintFilter::acceptNode(const DOMNode *node) const
{
    ...
    switch(node->getNodeType())
    {
        case DOMNode::ELEMENT_NODE:
        {
             ...
             * FILTER_REJECT: Reject the node. The Childer of this node will also be rejected.
...
//                return DOMNodeFilter::FILTER_REJECT;
            ...
            // removing children of body div,h2,body_text
            if(XMLString::compareString(node->getNodeName(), toXMLCh("div")) == 0)
            {
                return DOMNodeFilter::FILTER_REJECT;
            }

            if(XMLString::compareString(node->getNodeName(), toXMLCh("h2")) == 0)
            {
                return DOMNodeFilter::FILTER_REJECT;
            }

            if(XMLString::compareString(node->getNodeName(), toXMLCh("body_text")) == 0)
            {
                return DOMNodeFilter::FILTER_REJECT;
            }
        ...
        default :
        {
            return DOMNodeFilter::FILTER_ACCEPT;   // no effect
            break;
        }
    }
