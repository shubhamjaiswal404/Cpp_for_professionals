    ...
    if(mFile.exists())
    {
        if(mFile.open(QFile::ReadOnly)){
            while(!mFile.atEnd())
            {
                ...
                memset(pdfVer,'\0',sizeof(pdfVer) + 1);
                mFile.read(mBuffer,sizeof(mBuffer));
                i=0;
                while(mBuffer[i] != 0x0A)
                {
                    i++;
                }
                memcpy(pdfVer, mBuffer , i);

                ui->plainTextEdit->appendPlainText(QString(pdfVer));
                break;
            }
        }        mFile.close();
    }
    else{
        ui->statusbar->showMessage(" PDF File does not exist");
    }
